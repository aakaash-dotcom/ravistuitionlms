...
import { downloadPeriodReportCard, sharePeriodReportOnWhatsapp, pctOf, gradeFor } from '@/lib/pdf';
import BackBar from '@/components/BackBar';
import { BRAND } from '@/lib/brand';
import { Download, MessageCircle, Loader2, TrendingUp, FileText } from 'lucide-react';
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <>
          ...
          {mode === 'Daily MCQ' ? (
            /* MCQ results — simple list, no performance/report card */
            mcqResults.length === 0 ? (
            ...
            <>
              ...
              {/* Per-period report cards with download + share */}
              {periods.map((p) => {
                ...
                return (
                  <div key={p} className="card overflow-hidden">
                    <div className="bg-[#0052FF] text-white px-4 py-2 flex items-center gap-2">
                      ...
                      <span className="badge bg-white/20 text-white">{totalPct}% · {gradeFor(totalPct)}</span>
                    </div>
                    ...
                    <div className="p-3 flex gap-2">
                      <button onClick={() => downloadPeriod(p)} className="btn-primary flex-1 !py-2" disabled={downloading === p}>
                        {downloading === p ? <Loader2 size={16} className="animate-spin" /> : <><Download size={14} /> Download Report</>}
                      </button>
